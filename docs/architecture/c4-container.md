...
flowchart LR
    ...
    web -- "GET /localidades<br/>(UF + cidade)" --> ibge
    web -- "GET /search<br/>(bairros, debounced)" --> nom
```
...
| `apps/web` → Nominatim (OSM)    | GET `/search?q=<bairro>,<cidade>,<uf>,Brazil&format=jsonv2&...`             | HTTPS (público, sem auth; User-Agent obrigatório por fair-use)                                  | S35 (FE — autocomplete de bairro no form de cobertura; ver [ADR-019](../decisions/ADR-019-nominatim-bairros.md)) |
